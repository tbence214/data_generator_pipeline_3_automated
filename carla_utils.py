"""
World setup, projection math, and seeding utilities shared across the pipeline.
"""
import os
import random
...
def ensure_directories(cfg):
    for path in [cfg.capture.output_dir, cfg.capture.labels_dir,
                 cfg.capture.rgb_dir, cfg.capture.bbox_dir]:
...
def load_world(client, map_name):
    """Loads a CARLA map if one is specified, otherwise returns the current world."""
    if map_name:
...
def resolve_seeds(cfg):
    """
    Derives per-subsystem seeds from the master seed configuration.

    If no seeds are configured at all, a random master seed is generated so
    each run is unique by default. Individual seeds override the master for
    reproducible partial runs.
    """
    if (cfg.master_seed is None and cfg.traffic_seed is None
            and cfg.ego_seed is None and cfg.walker_seed is None):
        master = random.SystemRandom().randint(0, 2**31 - 1)
    else:
        master = (cfg.master_seed if cfg.master_seed is not None
